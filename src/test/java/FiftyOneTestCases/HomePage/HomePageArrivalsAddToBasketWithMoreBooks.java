...
public class HomePageArrivalsAddToBasketWithMoreBooks {
	...
	@Test(priority = 5)
	public void addMoreBooks() throws InterruptedException {
		
		driver.navigate().to("https://practice.automationtesting.in/product/mastering-javascript/");
		
		driver.findElement(By.xpath("//input[@class='input-text qty text']")).click(); //click the box
		driver.findElement(By.xpath("//input[@class='input-text qty text']")).sendKeys("4051"); //sendkey 4051 books
		driver.findElement(By.xpath("//button[@class='single_add_to_cart_button button alt']")).click();
		Thread.sleep(3000);
		System.out.println("its throws an error prompt");
